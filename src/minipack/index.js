 ...
 * [源码实现](https://github.com/sisterAn/minipack)
 * window 平台下开发，使用 replace(/\\/g, '\\\\') 转换路径
 */
const fs = require('fs')
...
function createGraph(entry) {
  ...
  function recursionDep(filename, assert) {
    ...
    assert.dependencies.forEach(relativePath => {
      ...
      // const a = { a: path.normalize(absolutePath) }
      // console.log(a)
      // console.log(a.a.replace(/\\/g,"\\"))
      ...
      if (!queue[absolutePath]) {
        ...
        queue[absolutePath.replace(/\\/g, '\\\\')] = child
...
function bundle(graph) {
  ...
  // !!! 将 require('relativePath') 转换为 require('absolutePath')
  const result = `
    (function(modules) {
